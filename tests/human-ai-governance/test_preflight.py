...
def init_repo(root: Path, marker: str = "0.7.1") -> None:
    run(["git", "init", "-q"], root)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="governance-v071-") as temp:
        base = Path(temp)
    ...
    print("PASS: v0.7.1 preflight behavior tests passed.")
    return 0
