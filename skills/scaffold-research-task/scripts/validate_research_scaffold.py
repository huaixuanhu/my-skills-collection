...
COMPATIBLE_GENERATOR_VERSIONS = ("0.1.0", "0.1.1", scaffold.SKILL_VERSION)
...
def options_from_manifest(target: Path, payload: dict[str, object]) -> scaffold.BuildOptions:
    ...
    compatible_generators = [
        {"skill": scaffold.SKILL_NAME, "version": version}
        for version in COMPATIBLE_GENERATOR_VERSIONS
    ]
    if payload["generated_by"] not in compatible_generators:
...
def validate_files(
    options: scaffold.BuildOptions, generator_version: str = scaffold.SKILL_VERSION
) -> int:
    expected = scaffold.expected_relative_paths(options)
    marker = f"Generated by {scaffold.SKILL_NAME} v{generator_version}"
    ...
    for relative in expected:
        ...
        if scaffold.TOKEN_RE.search(text):
...
def validate_target(value: Path) -> tuple[scaffold.BuildOptions, int]:
    ...
    return options, validate_files(options, payload["generated_by"]["version"])
