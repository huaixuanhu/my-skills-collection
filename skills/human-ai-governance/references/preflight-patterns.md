...
For a `v0.7.8` to `v0.7.9` migration, synchronize evidence-led diagnosis, claim-supporting validation, and diagnostic retention in existing rules. Contradictory evidence triggers reassessment; repeated attempts alone do not require a method change. Preserve authentication-secret exclusions and applicable major-consequence protections. The generic preflight changes only its version marker; no new retry, logging, or approval gate is introduced.
