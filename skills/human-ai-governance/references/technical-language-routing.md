...
Plain language does not mean a shorter or less complete answer. Remove or explain jargon only after preserving the information needed to execute, reproduce, verify, decide, or stop safely.
...
Use concrete descriptions of observable behavior when they preserve meaning. For example, “the operation is idempotent” may become “the operation is idempotent, so repeating it does not create an additional change.” Do not invent an analogy, causal explanation, or simplified rule that the evidence does not support.
