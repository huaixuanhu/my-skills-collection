#!/usr/bin/env python3
"""Reusable human-AI governance preflight scaffold.

Copy this file into a target repository and tune the constants for that project.
It is intentionally standard-library only.
"""
...
# These defaults intentionally exclude README, tests, and lockfiles. Tune them
# when a target project has a stronger definition of a material change.
MATERIAL_EXACT = {
    "AGENTS.md",
    "Dockerfile",
    "docker-compose.yml",
...
# Generic architecture checks can reliably notice topology changes, but they
# cannot infer semantic responsibility changes inside an existing file.
...
# Keep generic side-effect scanning away from docs, tests, and governance logs.
# Tune these paths before enabling --strict-side-effects in a target project.
RISK_SCAN_PREFIXES = (
    "app/",
    "api/",
    "lib/",
...
RISKY_SIDE_EFFECT_PATTERNS = (
    ...
    re.compile(r"\b(charge|payment|payout)\b", re.IGNORECASE),
    re.compile(r"\b(place|submit|modify)_order\b", re.IGNORECASE),
    re.compile(r"\b(transfer|withdraw)\b", re.IGNORECASE),
...
class GitInspectionError(RuntimeError):
    """Raised when repository state or a required staged blob cannot be inspected."""
...
def git_failure(action: str, stderr: str | bytes) -> GitInspectionError:
    if isinstance(stderr, bytes):
        ...
        detail = stderr
    first_line = next((line.strip() for line in detail.splitlines() if line.strip()), "unknown error")
...
def git_changed_paths(root: Path) -> ChangeSet:
    status = run_git_bytes(root, ["status", "--porcelain=v1", "-z", "--untracked-files=all"])
...
def read_index_text(root: Path, relative_path: str) -> str | None:
    ...
    if result.returncode != 0:
        raise git_failure(f"read staged content for {display_path(relative_path)}", result.stderr)
    try:
...
def build_snapshot_map(root: Path, changes: ChangeSet) -> dict[str, tuple[SnapshotText, ...]]:
...
def display_path(path: str) -> str:
    ...
    if path != path.strip() or any(character in path for character in "\0\n\r\t") or has_surrogate:
...
def validate_ai_log(text: str) -> list[str]:
    ...
    issues = [f"latest {AI_AGENT_LOG} entry lacks {field}" for field in required if field not in entry.body]
...
def has_structure_sensitive_change(entries: list[StatusEntry]) -> bool:
    for entry in entries:
        ...
        if "R" in entry.status and entry.original_path is not None:
...
def material_changed_paths(entries: list[StatusEntry], paths: set[str]) -> set[str]:
    ...
    material_paths.update(
        ...
        and entry.original_path is not None
        and is_material_path(entry.original_path)
    )
...
def validate_plan_parent_markers(
    ...
    for path in sorted(p for p in paths if is_plan_doc(p)):
        for snapshot in snapshots.get(path, ()):
            ...
            has_source = any(
                marker in head for marker in ("Source plan:", "来源计划", "Parent plan:")
            )
            has_derived = any(
                marker in head
                for marker in ("Derived from:", "衍生自", "Document nature:", "文档性质")
            )
...
def marker_versions(
    ...
    candidate_paths = ("AGENTS.md", "governance/AI_AGENT_LOG.md", "README.md")
...
def changed_marker_issues(
    ...
    issues: list[str] = []
    candidate_paths = {"AGENTS.md", "AGENTS.override.md", "governance/AI_AGENT_LOG.md", "README.md"}
    for path in sorted(candidate_paths.intersection(snapshots)):
        for snapshot in snapshots[path]:
            versions = {
                match.group("version") for match in SKILL_MARKER_RE.finditer(snapshot.text)
            }
            if versions and SKILL_VERSION not in versions:
                found = ", ".join(sorted(versions))
                issues.append(
                    f"skill marker is stale in {display_path(path)}{snapshot_note(snapshot)}: "
                    f"found {found}; expected {SKILL_VERSION}"
                )
    return issues
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"human-ai-governance preflight scaffold v{SKILL_VERSION}")
    ...
    if args.require_skill_marker:
        ...
        if not versions:
            ...
            found = ", ".join(sorted(versions))
            marker_issues.append(f"skill marker is stale: found {found}; expected {SKILL_VERSION}")
        else:
    ...
    if args.tier >= 2 and material_paths:
        if (root / CHANGELOG).exists():
            if CHANGELOG not in paths:
                ...
                issues.extend(
                    validate_snapshot_documents(CHANGELOG, snapshots, validate_changelog)
                )
        ...
        if material_paths:
            if (root / AI_AGENT_LOG).exists():
                if AI_AGENT_LOG not in paths:
                    ...
                    issues.extend(
                        validate_snapshot_documents(AI_AGENT_LOG, snapshots, validate_ai_log)
                    )
            else:
                issues.append(
    ...
    if args.strict_side_effects:
        ...
        for finding in scan_risky_side_effects(snapshots, material_paths):
            message = (
