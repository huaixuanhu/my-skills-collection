...
来源与状态：本文件直接响应本次用户提出的“合并现有内容、建立升级分支、深度调研”请求，保留调研结论与来源。用户随后确认在现有 skill 内升级，并强调注意力连续性和设计语言的长期维护；当前实施入口为衍生的 [v0.2.0 计划](../plans/design-authorship-v0.2.0.md)。第 8 节保留提案来源，执行状态以该计划和版本记录为准。
...
本节为初始调研建议。用户已确认在现有 `design-authorship` 内升级，保留 UI/幻灯片/图示路由和按需读取参考文件的结构；目标版本确定为 `v0.2.0`。注意力连续性与可维护统一设计语言成为本轮重点，具体实施与验证见 [衍生计划](../plans/design-authorship-v0.2.0.md)。
...
截至调研提交 `26c072d`，没有 push、创建 tag、修改安装链接或迁移下游项目，skill 本体保持合并后的版本。后续实施状态见第 8 节所链接的计划。
